...
def predictive_score_metrics(ori_data, generated_data):
  ...
  def predictor (x, t):
    ...
    with tf1.variable_scope("predictor", reuse=tf1.AUTO_REUSE) as vs:
      p_cell = tf.keras.layers.GRUCell(units=hidden_dim, activation='tanh')
      rnn_layer = tf.keras.layers.RNN(p_cell, return_sequences=True)
      mask = tf.sequence_mask(t, maxlen=tf.shape(x)[1])
      p_outputs = rnn_layer(x, mask=mask)
      y_hat_logit = tf.keras.layers.Dense(1, activation=None)(p_outputs)
      y_hat = tf.nn.sigmoid(y_hat_logit)
      p_vars = tf1.trainable_variables(scope=vs.name)
    return y_hat, p_vars 
